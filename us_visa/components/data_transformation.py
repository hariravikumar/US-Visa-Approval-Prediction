    ...
    def initiate_data_transformation(self, ) -> DataTransformationArtifact:
        ...
        try:
            if self.data_validation_artifact.validation_status:
                ...
                #Get Input feature and target feature from train dataset
                input_feature_train_df = train_df.drop(columns=[TARGET_COLUMN], axis=1)
                target_feature_train_df = train_df[TARGET_COLUMN]

                logging.info("Got train features and test features of Training dataset")

                #add company_age column to the Training dataset
                input_feature_train_df['company_age'] = CURRENT_YEAR-input_feature_train_df['yr_of_estab']

                logging.info("Added company_age column to the Training dataset")

                #get the columns set in schema_config to  drop from  Training dataset
                drop_cols = self._schema_config['drop_columns']

                logging.info("drop the columns in drop_cols of Training dataset")

                #drop the columns set in schema_config in drop_cols of Training dataset
                input_feature_train_df = drop_columns(df=input_feature_train_df, cols = drop_cols)

                #Convert the target feature to target value mapping for train dataset
                target_feature_train_df = target_feature_train_df.replace(
                    TargetValueMapping()._asdict()
                )

                #Get Input feature and target feature from test dataset
                ...
                #Convert the target feature to target value mapping for test dataset
